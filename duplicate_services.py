...
class get_all_fw_policy():
    ...
    def get_duplicate_services(self):
        resp_dup = self.showDuplicatesServices(None)
        return resp_dup.text
    ...
    def getService(self, id):
        Path = self.server_instance.root_url + "/api/juniper/sd/service-management/services/%s" % (id)
        header = {'Accept': 'application/vnd.juniper.sd.service-management.service+json;version=1;q=0.01'}
        response = self.server_instance.session.get(url=Path, headers=header, verify=False)
        return response
    ...
    def merge_dup_services(self, page):
...
if __name__ == "__main__":
    ...
    resp_status_show_dup = SD_API_Inst.get_duplicate_services()
    resp_status_show_dup = json.loads(resp_status_show_dup)

    for ids in resp_status_show_dup['duplicate-list']['duplicates']:
            dup_service_id_list = ids['idList']
            first_id = dup_service_id_list[0]
            service_get_resp = SD_API_Inst.getService(first_id)
            service_detail = json.loads(service_get_resp.text)
            #print(json.dumps(service_detail, indent=2))
            merge_service_name = service_detail['service']['name']
            print(merge_service_name)
            post_body = {
                "merge-request": {
                    "name": merge_service_name,
                    "description": "",
                    "is-group-merge": "false",
                    "ids": dup_service_id_list
                }
            }
            merge_service_resp = SD_API_Inst.merge_dup_services(post_body)
            ##Check POST response status message
            print(merge_service_resp)